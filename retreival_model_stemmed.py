...
f = open("./index_stat_stemmed.json")
...
for qid, query in query_details.items():
    ...
    for token_in_query in query_tokenized:
        #print("token_in_query",token_in_query)
        for doc_in_tok in inverted_index[token_in_query].keys():
            #print("doc_in_tok",doc_in_tok)
    ...
    for doc in tqdm(doc_to_search):
        ...
        # score_okapitf_list.append([score_okapitf, doc, qid])
        # score_tfidf_list.append([score_tfidf, doc, qid])
        # score_bm25_list.append([score_bm25, doc, qid])
        # if doc in term_present:
        #     score_lml_list.append([score_lml, doc, qid])
        #     score_lmjm_list.append([score_lmjm, doc, qid])
...
for key in tqdm(score.keys()):
    query_doc_es = [str(arr[2]) + " Q0 " + arr[1] + ' ' + str(i + 1) + ' ' + str(arr[0]) + ' Exp\n' for i, arr in
                    enumerate(score[key])]

    file = open( key + "_stemmed_res.txt", "w+")
