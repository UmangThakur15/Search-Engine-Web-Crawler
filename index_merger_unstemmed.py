...
f = open("./indexfiles_unstemmed/doc_id_map.json")
doc_id = json.load(f)
...
f = open("./words_id_dict_unstemmed.json")
word_id = json.load(f)
...
for i in tqdm(range(0,85)):
    with open("indexfiles_unstemmed/index/index"+ str(i) +".csv") as f:
        index = f.readlines()
        index = [i.strip() for i in index]

    with open("indexfiles_unstemmed/catalog/catalog"+ str(i) +".csv") as f:
        cat = f.readlines()
...
save_file = open("inverted_index_unstemmed.json", "w")
