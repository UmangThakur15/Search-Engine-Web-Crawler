...
#okapitf
def compute_otf(tfd, ld, avg_lg):
    return tfd / (tfd + 0.5 + (1.5 * (ld/avg_lg)))



#tfidf
def compute_tfidf(tfd, ld, avg_lg, dtf):
    return compute_otf(tfd, ld, avg_lg) * math.log(84678 / dtf)
...
#lmjm
def compute_lmjm(tfd, ld, ttf, vocab):
    l = 0.94
...
for query_id, query in queries.items():
    ...
    for doc in parsed_data:
        ...
        for token in query_tokenized:
            ...
            score_okapitf =score_okapitf + compute_otf(tf, ld, avg_lg)
            score_tfidf = score_tfidf + compute_tfidf(tf, ld, avg_lg, tdf)
            score_bm25 = score_bm25 + compute_bm(tf, ld, avg_lg, tdf, tfq)
            score_lml = score_lml + compute_lml(tf, ld, vocab)
            score_lmjm = score_lmjm + compute_lmjm(tf, ld, ttf, vocab)
    ...
    #print("okapitf :",score["okapitf"],"\n")

    score["tfidf"].extend(score_tfidf_list)
    # print("tfidf :",score["tfidf"],"\n")
    ...
    # print("lmjm :",score["lmjm"],"\n")


    #print(score_okapitf_list,"\n")
for key in score.keys():
    query_doc_es = [str(arr[2]) + " Q0 " + arr[1] + ' ' + str(i + 1) + ' ' + str(arr[0]) + ' Exp\n' for i, arr in
                    enumerate(score[key])]

    file = open(key + "_result.txt", "w+")
